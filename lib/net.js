...
Socket.prototype._destroy = function(exception, cb) {
  ...
  for (var cur = this; cur !== null; cur = cur._parent) {
    if (!cur.server)
      continue;

    COUNTER_NET_SERVER_CONNECTION_CLOSE(cur);
    debug('has server');
    cur.server._connections--;
    if (cur.server._emitCloseIfDrained) {
      cur.server._emitCloseIfDrained();
    }
  }
};
