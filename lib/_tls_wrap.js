...
function loadSession(self, hello, cb) {
  ...
  if (hello.sessionId.length <= 0 ||
      hello.tlsTicket ||
      self.sslServer &&
      !self.sslServer.emit('resumeSession', hello.sessionId, onSession)) {
    cb(null);
  }
}
...
function requestOCSP(self, hello, ctx, cb) {
  if (!hello.OCSPRequest || !self.sslServer)
    return cb(null);

  if (!ctx)
    ctx = self.sslServer._sharedCreds;
  if (ctx.context)
    ctx = ctx.context;

  if (listenerCount(self.sslServer, 'OCSPRequest') === 0) {
    return cb(null);
  } else {
    self.sslServer.emit('OCSPRequest',
                        ctx.getCertificate(),
                        ctx.getIssuer(),
                        onOCSP);
  }
...
function onnewsession(key, session) {
  ...
  if (!this.sslServer.emit('newSession', key, session, done))
    done();
...
TLSSocket.prototype._init = function(socket, wrap) {
  ...
  this.sslServer = options.server || null;
  ...
  if (options.isServer) {
    ...
    if (this.sslServer &&
        (listenerCount(this.sslServer, 'resumeSession') > 0 ||
         listenerCount(this.sslServer, 'newSession') > 0 ||
         listenerCount(this.sslServer, 'OCSPRequest') > 0)) {
...
function SNICallback(servername, callback) {
  ...
  this.sslServer._contexts.some(function(elem) {
